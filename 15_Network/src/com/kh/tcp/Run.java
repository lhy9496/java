 ...
 * 
 * IP 주소 : 네트워크 상에 각 컴퓨터들을 식별해줄 수 있는 번호(실제 주소와 동일한 역할)
 * 
 * * 서버와 클라이언트
 * > 서버 : 클라이언트(고객)에게 서비스를 제공해주는 프로그램 또는 컴퓨터(클라이언트의 요청에 응답을 해주는 것)
 * > 클라이언트 : 서버에 요청을 하는 컴퓨터 (서비스를 제공받는 고객)
 * 
 * - 서버에 요청하기 위해서는 그 요청하고자 하는 서버의 IP 주소(또는 도메인), 포트번호를 알아야 함
 * 
 *  * 현재 구동중인 서버가 있을 때
 *    클라이언트는 그 서버로부터 요청을 보낼 수 있음 => 응답 결과가 돌아옴
 *    
 *    - 요청과 응답에 의해서 웹은 작동한다.("통신한다")
 *    - 웹에서의 통신방식 : HTTP 프로토콜을 사용한다.
 *    
 *    * 자바만 가지고 서버와 클라이언트 간에 간단한 통신 해보기
 *    이때 데이터를 입출력하고자 한다면 서버와 클라이언트 간에 스트림(연결통로)이 필요함
 *    
 *    * 소켓 : 프로세스간의 통신을 담당
 *    
 *    * 소켓 프로그래밍 (TCP / UDP)
 *    - TCP방식 : 데이터 전송 속도가 느리지만, 데이터가 정확하고 안정적으로 전달이 가능(신회성이 요구되는 프로그램)
 *    - UDP방식 : 데이터 전송 속도가 빠르지만, 신뢰성 없는 데이터 전달에 용이(데이터를 빠른 속도로 전송하고자 할 때)
 */
...
public class Run {
	public static void main(String[] args) {
		try {
			...
			InetAddress googleHost = InetAddress.getByName("www.google.com"); // 도메인명을 통해서 해당 호스트에 정보를 가져옴
