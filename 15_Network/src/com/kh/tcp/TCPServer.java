...
public class TCPServer {
	 ...
	 *  - 클라이언트간에 1:1 소켓통신
	 *  - 데이터를 교화하기에 앞서 서버, 클라이언트 연결이 되어야 한다.(서버가 먼저 실행되어 클라이언트
	 *  - 신뢰성있는 데이터 전달가능
	 *  
	 *  * Socket
	 *  - 프로세스간의 통신을 담당
	 *  - Input/OutputStream을 가지고 있다.(스트림을 통해 입출력이 이루어짐)
	 *  
	 *  * ServerSocket
	...
	public static void main (String[] arge) {
		...
		// 1) 포트번호 지정(서버측에서 몇번 포트로 통로를 열겠냐는 의미)
		int port = 3000;
		
		ServerSocket server;
		try {
			// 2) ServerSocekt 객체생성 시 포트 결합(Bind)
			server = new ServerSocket(port);
			
			// 3) 클라이언트로부터 접속 요청이 올 때까지 대기
			System.out.println("클라이언트 요청을 기다리고 있습니다.");
			
			// 4) 연결 요청이 오면 => 요청 수락 후 클라이언트와 통신할 수 있는 서버 측 소켓객체 생성
			Socket socket = server.accept();
			System.out.println(socket.getInetAddress().getHostAddress() + "가 연결을 요청함...");
			
			// 5) 클라이언트와 입출력 기반스트림 생성(바이트스트림밖에 안 됨)
			// 6) 보조스트림으로 성능 개선
			
			//입력용 스트림(클라이언트로부터 전달된 값을 한줄 단위로 읽어들이기 위한 입력용 스트림)
			br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
			
			//출력용 스트림(클라이언트에게 값을 한줄 단위로 출력하기 위한 출력용 스트림)
